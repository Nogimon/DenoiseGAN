...
def generator(gen_inputs, gen_output_channels, reuse=False, FLAGS=None):
    ...
    with tf.variable_scope('generator_unit', reuse=reuse):
        ...
        '''
        #pooling layer before last
        with tf.variable_scope('pooling1'):
            net = slim.max_pool2d(net, [2, 2], scope='pool1')
        '''
        '''
        with tf.variable_scope('downconv'):
            net = conv2(net, 3, 256, 2, scope = 'conv')
            net = batchnorm(net, FLAGS.is_training)
        '''
        ...
        with tf.variable_scope('downconv1'):
            net = conv2(net, 3, 256, 2, scope = 'conv')
            net = batchnorm(net, FLAGS.is_training)

        with tf.variable_scope('downconv2'):
            net = conv2(net, 3, 64, 2, scope = 'conv')
            net = batchnorm(net, FLAGS.is_training)

        net = net + stage2_output
        '''
